...
births_excl = births_excl %>%
  mutate(GEST4 = GEST^4)
model4 = lm(data = births_excl, BWTG ~ GEST + GEST2 + GEST3 + GEST4 + PARITY_truncated + PLUR + smoking_type + MAGE + MRACER + mortality, na.action = "na.exclude")
summary(model4)
plot(model4$fitted.values, model4$residuals)
plot(births_excl$GEST, model4$residuals)
plot(births_excl$PLUR, model4$residuals)
