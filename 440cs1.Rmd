...
births %>% filter(smoked_during) %>% select(total_smoked) %>% unlist() %>% mean()
...
summary(mage_reg)
```
Mother's age seems to be fairly normally distributed with a mean of 27.8. There does appear to be a bit of a long right tail, as most mothers are between the ages of 17 and 30 but there are also mothers who are as old as 50. There appears to be a positive relationship between the age of the mother and the birth weight. There is no evidence to suggest that the birth weight variance is not constant across the mother's age.
