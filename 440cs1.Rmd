...
In this report, we will be investigating the relationship between birthweight and various factors. The data we will be analyzing includes children born in North Carolina between 2011 and 2016, totaling 720 thousand observations. In addition to birthweight, this data includes factors such as sex, gestation period, smoking behavior, mother's race, and county of residence, which we used as a proxy for mother's socioeconomic status. We will first explore each of these variables and determine which factors are relevant to birthweight, then use the relevant factors to create and evaluate a linear regression model. We will also use M-estimation to create a more robust model. Finally, we will use cross-validation to compare our chosen linear model with the more robust version.
...
The addition of the quartic term does seem to help a bit, and it seems that we've gone as far as we can go with polynomial terms on gestation. Despite this, our assumption of normality still does not appear to be totally met, as shown by tbe Q-Q plot, and the residuals are not quite homoskedastic, as the variance of the residuals increases as the fitted values gets larger. Because of this, we should look into more robust methods. For that we will utilize M-estimation.
