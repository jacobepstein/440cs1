...
summary(parity_reg)
```
We see that parity does have a significant impact on birth weight. Independent of other variables, the second child has the highest median birth weight and then the median birth weight slightly decreases for later children. Specifically, the sixth child has a lower median birth weight than the first child. We chose to group parities of five or more as it provides more concise interpretation and prevents overfitting for parities that have very few data points. We see that the frequency of parity seems to decrease in an exponential fashion.
...
summary(gest_reg)
```
The relationship appears to be cubic. We see th most common gestational period is 39 weeks and the period with the highest media expected weight is 42 weeks.
...
ggplot(data = births, mapping = aes(x = MAGE, y = BWTG)) +
  ...
  ylab("Birth weight (g)") + 
  geom_smooth(method='lm',formula=data$BWTG~data$MAGE) + 
  geom_point() +
  ggtitle("NC Births, 2011-2016") + 
  theme_minimal()
